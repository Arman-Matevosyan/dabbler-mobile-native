...
export const PaymentAPI = {
  ...
  getSubscriptions: async (): Promise<ISubscription> => {
    const response = await client.get<ApiResponse<ISubscription>>(
  ...
  getPaymentMethods: async (): Promise<IPaymentMethod[]> => {
    const response = await client.get<ApiResponse<IPaymentMethod[]>>(
  ...
  subscribe: async (
    ...
    >('/payment/subscriptions', subData);
    return response.response;
  },

  getClientToken: async (): Promise<IPaymentGatewayToken[]> => {
    const response = await client.get<ApiResponse<IPaymentGatewayToken[]>>(
